...
interface BureauControlsProps {
    className?: string
    orientation?: 'horizontal' | 'vertical'
    showWeather?: boolean
}

export const BureauControls: React.FC<BureauControlsProps> = ({ className = '', orientation = 'horizontal', showWeather = true }) => {
    ...
    return (
        <div className={`flex ${orientation === 'vertical' ? 'flex-col items-start' : 'items-center'} gap-4 ${className}`}>
            {/* Weather Controls */}
            {showWeather && (
            ...
            <div className="flex items-center gap-4 p-2 pl-4 pr-4 rounded-full bg-black/20 backdrop-blur-md border border-white/5">
                <div className="flex items-center gap-2 mr-2">
                    ...
                    <div className="flex flex-col w-32 overflow-hidden">
                        <span className="text-xs font-bold text-white text-outline whitespace-nowrap truncate">
                            {isRadioOn ? currentStation.name : "Radio OFF"}
                        </span>
                        <span className="text-[10px] text-white/80 text-outline whitespace-nowrap truncate">
                ...
                <div className="flex items-center gap-2 w-20 group">
                    ...
                    <div className="flex-1">
                        <Slider
                            ...
                            max={100}
                            step={1}
                            value={Math.round(Math.pow(Math.min(volume, 0.5) / 0.5, 1 / 1.74) * 100)}
                            onChange={(val) => {
                                if (typeof val !== 'number') return
                                const linear = val / 100
                                const curved = Math.pow(linear, 1.74)
                                setVolume(curved * 0.5)
                            }}
